...
class AWP_Jobs extends AWP_Base
{
	...
	function save_applicantjobs($formname,$jobId,$jobNo){
		...
		if(!empty($hrjobsform)){
			...
			$industryId = sanitize_text_field($submittedformvalues['industry']);
			...
			if(!empty($emailId)){
				...
				$response = saveCandidate($firstName,$lastName,$emailId,$phoneNumber,$upload_docid,$coverletter,$jobTitle,$comments,$industryId,$address1,$address2,$city,$provinceAndState,$postalCode,$country,$documentId,$skills);

                $jobApplicantId	= $response->applicantId;
                
                if($jobApplicantId !="" && $upload_docid != ""){
                	 $file_ext = strtolower( end(explode('.',$upload_docid)));
                         $file_size= $_FILES['file_upload']['size'];
                         $file_tmp= $_FILES['file_upload']['tmp_name'];
                         $data = file_get_contents($file_tmp);
                         $base64 = base64_encode($data);
                         $add_document=uploadDocument($jobApplicantId,$upload_docid,$file_ext,$file_size,$base64);
...
function saveCandidate($firstName,$lastName,$emailId,$phone,$resumeName,$coverLetter,$jobTitle,$comments,$industryId,$address1,$address2,$city,$state,$zipcode,$country,$documentId,$skills) {
        
        if(empty($country)) $country = 176;
        if(empty($industryId)) $industryId = '-1';
       ...
       $addressData = '["-1","'.$country.'","'.$address1.'","'.$address2.'","","'.$city.'","'.$state.'","'.$zipcode.'","'.$country.'"]';
        ...
        $response = getRestAPICall("POST", APPTIVO_CANDIDATE_API, $param);
        return $response;
}
...
* upload document
*/
function uploadDocument($candidate_id,$file_name,$file_ext,$file_size,$base64) {
    $params = array(
        "a" => "uploadDoc",
        "objectId" => APPTIVO_JOBS_OBJECT_ID,
    	"objectRefId"=> $candidate_id,
    	"docName"=> $file_name,
        "docTitle"=> "",
    	"docType"=> $file_ext,
    	"docSize"=> $file_size,
